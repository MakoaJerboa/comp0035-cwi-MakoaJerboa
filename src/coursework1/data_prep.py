...
csvfile = Path(__file__).parent.parent.parent.joinpath("data", "dataset.csv")
...
for i, category in enumerate(worker_categories):
    ...
    category_df = df_dropped[df_dropped['hours_worked'] == category]
    
    # Counts the number of people in each worker caterory and sorts it by area
    category_count = category_df.groupby('lad22nm').size().reset_index(name=worker_categories_friendly[i])
    
    # Since the area labels are constant, this section is only run once
    if i == 0:
        # Takes the only first column and gives it a simpler label
        df_prepared = category_count.rename(columns={"lad22nm": "Area"}).drop(category_count.columns[[1]], axis=1)
...
df_prepared.to_csv(Path(__file__).parent.parent.parent.joinpath("data", "dataset_prepared.csv"), index=False)